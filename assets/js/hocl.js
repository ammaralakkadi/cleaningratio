/* =====================================================
   hocl.js
   Hypochlorous Acid (HOCl) Dilution Calculator – Logic
   Pure JS, no dependencies
===================================================== */

document.addEventListener("DOMContentLoaded", () => {
  const form = document.getElementById("hocl-dilution-form");
  const calculateBtn = document.getElementById("calculate-hocl-btn");
  ...
  const resultCard = document.getElementById("hocl-result-card");
  ...
  calculateBtn.addEventListener("click", () => {
    ...
    if (stockRaw <= 0) {
      resultCard.hidden = true;
      showFeedback("Stock HOCl strength must be more than 0.");
      return;
    }
    ...
    if (targetPpm === stockPpm) {
      resultCard.hidden = true;
      showFeedback("Target ppm equals stock strength — no dilution needed.");
      return;
    }
    ...
    if (targetPpm > stockPpm) {
      resultCard.hidden = true;
      showFeedback(
        "Target ppm must be lower than stock strength to calculate a dilution.",
      );
      return;
    }
